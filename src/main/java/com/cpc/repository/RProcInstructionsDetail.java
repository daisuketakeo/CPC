...
/**
 * 工程別製造指図詳細テーブル repository
 */
@Mapper
@Component
public interface RProcInstructionsDetail{
 
    @Insert("INSERT INTO PROC_INSTRUCTIONS_DETAIL_TABLE ( "
            + "  BATCH_ID "
            + ", INS_PROC_ID "
            + ", INS_PROC_DETAIL_ID "
            + ", ITEM_CONDITION "
    ...
    int insert(PROC_INSTRUCTIONS_DETAIL_TABLE model);
    
    @Update({
        "<script>",
        "UPDATE PROC_INSTRUCTIONS_DETAIL_TABLE ",
        "  <set>",
    ...
    int update(PROC_INSTRUCTIONS_DETAIL_TABLE model);
    
    @Delete("DELETE FROM PROC_INSTRUCTIONS_DETAIL_TABLE "
         +" WHERE "
            + "     BATCH_ID = #{BATCH_ID} "
            + " AND INS_PROC_ID = #{INS_PROC_ID} "
            + " AND INS_PROC_DETAIL_ID = #{INS_PROC_DETAIL_ID} "
         )
    int delete(PROC_INSTRUCTIONS_DETAIL_TABLE model);

    @Select({
     "<script>",
        "SELECT * FROM PROC_INSTRUCTIONS_DETAIL_TABLE ",
        "  <where>",
    ...
    List<PROC_INSTRUCTIONS_DETAIL_TABLE> select(PROC_INSTRUCTIONS_DETAIL_TABLE model);
